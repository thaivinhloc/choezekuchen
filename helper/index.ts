...
const teachingDetailRegex = /^([a-z0-9-\/]*)-t([0-9]*).html$/;
...
export const getPathType = (path: string) => {
  ...
  if (teachingDetailRegex.test(path)) {
    return "teaching";
  }
...
export const getTeachingDetailPathFromSlug = (
  id: number,
  slug = ""
): string => {
  if (slug.startsWith("/")) {
